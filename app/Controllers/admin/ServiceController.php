    ...
    public function save () {
       ...
       //1Get current DB variants for this service
        ...
        $data = [
            ...
            'variant_title'   => 'kalami Demo',//$this->request->getPost('variant_main_title'),
        ];
        if(!empty($imagePath)) {
        ...
        if($data) {
            foreach($data as $row) {
                ...
                if(!isset($dataPoints[$rowId])) {
                    ...
                    if(!empty($row['varintId'])) {
                        $dataPoints[$rowId]['variants'][] = [
                            ...
                            'image'    => ($row['varintImg'] ? base_url($row['varintImg']) : base_url('uploads/default.png'))
                        ];
                    }
                }
                else
                {
                    ...
                    if(!empty($row['imgId'])) {
                        $exsitingGalleryId = array_column( $dataPoints[$rowId]['gallery'],'imgId');
                        if(!in_array( encryptor($row['imgId']),$exsitingGalleryId)) {
                            $dataPoints[$rowId]['gallery'][] = [
                                'img'      => $row['image_url'],
                                'imgId'  => encryptor($row['imgId'])
                            ];
                    ...
                    if(!empty($row['varintId'])) {
                        $variantExisting = array_column($dataPoints[$rowId]['variants'],'varintId');
                        if(!in_array($row['varintId'],$variantExisting)) {
                            $dataPoints[$rowId]['variants'][] = [
                                'varintId' => $row['varintId'],
                                'varintTitle' => $row['varinttitle'],
                                'varintdescription' => $row['variantdesc'],
