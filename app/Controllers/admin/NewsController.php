...
class NewsController extends Controller {
    ...
    protected $newshighlights;
    protected $newsimgModel;

    function __construct() {
        $this->newsModel = new NewsModel();
        $this->imgUploader = new UploadImages();
        $this->newshighlights = new NewshighlightsModel();
        $this->newsimgModel = new NewsimageModel();
    }
     public function index() {
        $page = (hasPermission('','news') ? ' News' : lang('Custom.permissionDenied'));
        $route = (hasPermission('','news') ? 'admin/news/index' :'admin/pages-error-404');
    ...
    public function save () {
        ...
        if($id) {
            ...
            $this->newshighlights->where('news_id', $id)->delete();
            foreach($points as $point) {
                if(!empty($point)) {
                    $this->newshighlights->insert(['news_id' => $id,'points' => $point]);
                }
            }
            if(!empty($uploadedPaths)) {
                    foreach ($uploadedPaths as $url) {
                        $this->newsimgModel->insert(['news_id'   => $id,'image_url' => $url]);
                    }
                }
            ...
            if($insertId = $this->newsModel->insert($data)) {
                if(!empty($pointsData)) {
                    foreach($pointsData as &$key) {
                        $key['news_id'] = $insertId;
                    }
                }
                if(!empty($uploadedPaths)) {
                    foreach ($uploadedPaths as $url) {
                        $this->newsimgModel->insert([
                            'news_id'   => $insertId,
                            'image_url' => $url
                        ]);
                    }
                }
                $this->newshighlights->insertBatch($pointsData);
                $validStatus = true;
    ...
    function list(){
        ...
        $data = $this->newsModel->getData($search);
        $dataPoints= [];
        if($data) {
    ...
    public function getinfo($id=false) {
        $data = $this->newsModel->getData(decryptor($id));
        $dataPoints= [];
        if($data) {
    ...
    public function delete($id) {
        ...
        $status = false;
        $msg = '';
        $item = $this->newsModel->find(decryptor($id));
        if($item) {
            if($this->newsModel->where(['id' => decryptor($id)])->delete()) {
                $status  = true;
    ...
    public function glleryDelete($id) {
        ...
        $status = false;
        $msg = '';
        $item = $this->newsimgModel->find(decryptor($id));
        if($item) {
            if($this->newsimgModel->where(['id' => decryptor($id)])->delete()) {
                $status  = true;
                $msg = 'Successfully Deleted';
            }else{
                $msg = '!Opps try agian';
