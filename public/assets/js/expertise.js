function openExpertiseModal(id=false) {
    toggleModal('expertiseModal', true);
    let modal = $('#expertiseModal');
    modal.find('.head').text('Add New Expertise');
    const webForm = document.getElementById('expertiseForm');
    $('#edit_id').val('');
    ...
    if(id) {
        $('#edit_id').val(id)
       fetch(App.getSiteurl() +'admin/edit-expertise/'+`${id}`)
       .then(res => res.json())
       .then(result =>{
        if(result) {
            $('#expertiseModal').find('.head').text('Edit Expertise');
            webForm.querySelector('#title').value = result.title;
             if(result.image) {
                $('#selectedPreview').removeClass('hidden');
                webForm.querySelector('#previewImg').src = result.image ? result.image : App.getSiteurl() + 'uploads/slider/default.jpg';
            }
        }
       })    
    }
...
function loadExpertise(search = '') {
    
    $.ajax({
        url: App.getSiteurl() +'admin/expertise/data',
        method : 'POST',
        data:{search:search},
        dataType: 'json',
    ...
    if(response.result.length ===0){
            ...
            <table class="min-w-full border border-gray-200 divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-100 text-gray-600 uppercase text-xs">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-medium">IMG</th>
        ...
        paginatedData.forEach(function(item, idx) {
            html += `<tr>
                ...
                <td class="px-2 py-2 whitespace-nowrap border border-gray-300">
                    <button class="btn btn-sm btn-primary edit-expertise" onclick="openExpertiseModal('${item.encrypted_id}')">Edit</button>
...
$('#expertiseForm').on('submit', function(e) {
    ...
    $.ajax({
        url : App.getSiteurl() +'admin/expertise/save',
        method:'POST',
        data: formData,
        contentType: false,
        processData: false,
...
function deleteItem(e){
    ...
    if(confirm('are you sure You want to Delete This')) {
        if(id) {
            $.ajax({
                 url : App.getSiteurl()+'admin/expertise/delete/'+ id,
                type: "DELETE",
                dataType: 'json',
                success:function(response)
                { 
