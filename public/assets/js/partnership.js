function openExpertiseModal(id=false) {
    toggleModal('partnershipModal', true);
    let modal = $('#partnershipModal');
    modal.find('.head').text('Add New Partner');
    const webForm = document.getElementById('partnershipForm');
    $('#edit_id').val('');
    ...
    if(id) {
        $('#edit_id').val(id)
       fetch(App.getSiteurl() +'admin/edit-partnership/'+`${id}`)
       .then(res => res.json())
       .then(result =>{
        if(result) {
            $('#partnershipModal').find('.head').text('Edit Partners');
...
function loadExpertise(search = '') {
    ...
    $.ajax({
        url: App.getSiteurl() +'admin/partnership/data',
        method : 'POST',
        data:{search:search},
        dataType: 'json',
...
$('#partnershipForm').on('submit', function(e) {
    ...
    $.ajax({
        url : App.getSiteurl() +'admin/partnership/save',
        method:'POST',
        data: formData,
        contentType: false,
        processData: false,
...
function deleteItem(e){
    ...
    if(confirm('are you sure You want to Delete This')) {
        if(id) {
            $.ajax({
                 url : App.getSiteurl()+'admin/partnership/delete/'+ id,
                type: "DELETE",
                dataType: 'json',
                success:function(response)
                { 
